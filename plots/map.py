...
class Map:
    ...
    def wgs84_to_web_mercator_df(self, df:pd.DataFrame, lon:str, lat:str)->pd.DataFrame:
    ...
    def get_plot(self):
        # if parameter is set, then create a color column
        if 'parameter' in self.cfg:
            df = helper.aggregate_data(source=self.data,
                                       group_cols=st.session_state.project.stations_columns_df().columns,
                                       val_col=MP.NUMERIC_VALUE.name.lower(),
                                       agg_func=self.cfg['aggregation'])
            tooltips = f"""Station: @identifier<br>
                        {self.cfg['parameter']}: @{{{st.session_state.config.value_col}}}{{0.00}}"""
        else:
            df = self.data
            tooltips = f"Station: @station_identifier"
        
        df = self.wgs84_to_web_mercator_df(df, self.cfg['long'], self.cfg['lat'])
