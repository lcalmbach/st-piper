...
def show_summary():
    def calc_stats(df):
        par_col = st.session_state.key2col()[cn.PARAMETER_COL]
        val_col = st.session_state.key2col()[cn.VALUE_NUM_COL]
        if st.session_state.col_is_mapped(cn.CATEGORY_COL):
            category_col = st.session_state.key2col()[cn.CATEGORY_COL]
            par_group = [category_col, par_col]
        else:
            par_group = [par_col]
        if st.session_state.col_is_mapped(cn.UNIT_COL):
            par_group.append(st.session_state.key2col()[cn.UNIT_COL])
    ...
    def filter_stats(df):
        with st.sidebar.expander('🔎 Filter'):
            sel_stations = st.multiselect('Stations', st.session_state.get_station_list())
            if len(sel_stations) > 0:
                df = df[df[st.session_state.key2col()[cn.STATION_IDENTIFIER_COL]].isin(sel_stations)]
            if st.session_state.col_is_mapped(cn.CATEGORY_COL):
                par_cat_col = st.session_state.key2col()[cn.CATEGORY_COL]
    ...
    filtered_data = filter_stats(st.session_state.row_value_df)
...
def show_filters(df):
    ...
    with st.sidebar.expander("🔎 Filter"):
        ...
        parameter_options = st.session_state.key2par().values()
        columns_options = st.session_state.key2col().values()
        ...
        ##df_sample = df[st.session_state.get_parameter_detail_form_columns()]
...
def show_plots(df, parameter, show_histo, show_time_series):
    def get_histo_plot():
        ...
        cfg['value_col'] = st.session_state.value_col
    ...
    def get_time_series_plot():
        ...
        cfg['legend_col'] = st.session_state.parameter_col
        
        ts = Time_series(df, cfg)
        return ts.get_plot()

    df = df.sort_values(st.session_state.date_col)
...
def show_detail():
    ...
    value_col = st.session_state.key2col()[cn.VALUE_NUM_COL]
    par_list = list(st.session_state.parameter_map_df.index)
    ...
    field_list = st.session_state.column_map_df.index
    show_guideline= False
    show_histogram = False
    show_time_series = False
    with st.sidebar.expander("⚙️ Settings"):
        fields = st.multiselect("Show columns", field_list)
        # guideline_options = list(st.session_state.guidelines_df['name'])
        # if len(guidelines) > 0:
    ...
    date_fmt = st.session_state.date_format
